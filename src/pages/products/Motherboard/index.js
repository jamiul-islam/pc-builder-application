...
const MotherBoardPage = ({ products }) => {
  // filter products by motherboard
  const homeData = products.filter(
  ...
  return (
    <>
      ...
      >
        <h1 style={{ margin: "2vh 0" }}>Motherboard</h1>
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          {homeData.map((product) => (
            <Cards key={product._id} product={product} category="motherboard" />
          ))}
        </div>
      </div>
    </>
